...
class ChatRequest(BaseModel):
    ...
    mode controls which tool the orchestrator invokes:
      ...
      "generate_problem" → generate a complete coding problem from a prompt (teacher)
      "generate_tests"   → generate test cases for a problem description (teacher)
    ...
    user_message: str = Field(
        ..., min_length=1, description="The student's message, answer, or generation prompt"
    )
    current_code: str = Field(
        "", description="Latest snapshot of the student's code (optional for generation modes)"
    )
    compiler_output: str = Field(
        "", description="stdout + stderr from the code runner"
    )
    problem_description: str = Field(
        "", description="Full problem statement shown to the student (also used as input for generate_tests)"
    )
    ...
    mode: Literal["hint", "chat", "viva_start", "viva_answer", "generate_problem", "generate_tests"] = Field(
        "chat",
        description=(
            "hint=Socratic hint, chat=follow-up dialogue, "
            "viva_start=begin verification, viva_answer=submit answer, "
            "generate_problem=AI problem gen, generate_tests=AI test-case gen"
        ),
    )
    language: str = Field(
        "python",
        description="Programming language: python | java | cpp | javascript"
    )
    viva_session_id: Optional[str] = Field(
        None,
        description="Active viva session ID — passed by client to bypass LangGraph checkpoint lookup"
    )
    starter_code: Optional[str] = Field(
        None,
        description="System-provided boilerplate shown to the student (function signature, class wrapper, main). Used to distinguish student logic from scaffolding."
    )
    # ── Generation-specific fields (only used for generate_problem / generate_tests) ──
    generate_difficulty: str = Field(
        "intermediate",
        description="Difficulty for problem generation: beginner | intermediate | advanced"
    )
    sample_solution: str = Field(
        "",
        description="Optional reference solution for test-case generation"
    )
    num_cases: int = Field(
        8, ge=2, le=20,
        description="Number of test cases to generate (generate_tests mode)"
    )
...
class ChatResponse(BaseModel):
    ...
    generate_data: Optional[Dict[str, Any]] = Field(
        None,
        description="Generated problem or test-case suite (only populated for generate_problem / generate_tests modes)"
    )
