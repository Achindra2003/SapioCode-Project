...
def generate_questions(code: str, analysis: ASTAnalysisResult, num: int = 3) -> List[VivaQuestion]:
    ...
    # Function-profile questions — skip system-provided boilerplate functions
    _BOILERPLATE_FUNCTIONS = {
        "main", "Main", "run", "setup", "teardown", "__init__",
    }
    student_profiles = [
        fp for fp in analysis.function_profiles
        if fp.name not in _BOILERPLATE_FUNCTIONS
    ]
    for fp in student_profiles[:2]:
    ...
    # Issue-focused question — skip issues about system-provided boilerplate
    _BOILERPLATE_PATTERNS = ("main", "Main", "public static void", "int main", "String[] args")
    student_issues = [
        loc for loc in (analysis.issue_locations or [])
        if not any(bp in (loc.description or "") for bp in _BOILERPLATE_PATTERNS)
        and not any(bp in (loc.code_snippet or "") for bp in _BOILERPLATE_PATTERNS)
        and not (
            loc.line and loc.line <= len(code_lines)
            and any(bp in code_lines[loc.line - 1] for bp in _BOILERPLATE_PATTERNS)
        )
    ]
    if student_issues:
        loc = student_issues[0]
